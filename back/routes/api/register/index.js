...
router.post('/', (req, res) => {
    ...
    User.find({ id: u.id }) // id를 만드는 단계이기 때문에 _id가 없으므로 id로 하는게 맞다
        .then(r => {
            if (r) throw new Error('이미 등록되어 있는 아이디입니다.')
            return User.create(u)
        })
        .then(r => {
            const pwd = crypto.scriptSync(r.pwd, r._id.toString(), 64, { N: 1024 }).toString('hex')
        ...
        .catch(e => {
            res.send({ success: false, msg: e.massage })
        })
})
